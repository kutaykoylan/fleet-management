...
@Component
public class TransferCenterShipmentStrategy extends ShipmentStrategyTemplate {
    ...
    public void unloadIfSack(List<Delivery> deliveryWithStateList, Delivery delivery) {
        ...
        if(aSack.isPresent()){
            ...
            if(aSack.get().getDeliveryPoint().getId() == 3 ){
                packageWithSackHelper.unloadAllPackagesInSack(sackEntity);
            }else {
                failedPairLogHelper.logFailedPair(delivery.getBarcode(), "3");
    ...
    public void unloadIfPackage(List<Delivery> deliveryWithStateList, Delivery delivery) {
        ...
        if(aPackage.isPresent()){
            ...
            if(aPackage.get().getDeliveryPoint().getId() == 3  ){
                packageWithSackHelper.unloadPackageWithSack(packageEntity);
            }else{
